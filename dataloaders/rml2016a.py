...
class RML2016aDatasetWithNet(Dataset):
    def __init__(self, cut_len: int, net: nn.Module):
        class_num = 11
        sample_num = 220000

        dataset_dict = scio.loadmat(dataset_path)
        self.data = torch.from_numpy(dataset_dict['data']).float()[:, :, :cut_len]
...
class RML2016aCutOffDataset(Dataset):
    def __init__(self, cut_len: int):
        ...
        signals = torch.from_numpy(dataset_dict['data']).float()

        self.data = signals[:, :, :cut_len]
        self.labels = signals[:, :, cut_len:]
