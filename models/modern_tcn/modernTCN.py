...
import math
from models.modern_tcn.RevIN import RevIN
from models.modern_tcn.ModernTCN_Layer import series_decomp, Flatten_Head
...
class ModernTCN(nn.Module):
    def __init__(self,task_name,patch_size,patch_stride, stem_ratio, downsample_ratio, ffn_ratio, num_blocks, large_size, small_size, dims, dw_dims,
        ...
        if self.task_name == 'classification':
            ...
            self.head_class = nn.Linear((self.n_vars * self.head_nf) // 4,self.class_num)
...
if __name__ == "__main__":
    model = ModernTCN(task_name="classification", patch_size=8, patch_stride=4, stem_ratio=None,
                           downsample_ratio=2, ffn_ratio=1, num_blocks=[1, 1],
                           large_size=[13, 13], small_size=[5, 5], dims=[32, 64], dw_dims=[64, 64],
                           nvars=2, small_kernel_merged=False,
                           backbone_dropout=0.1, head_dropout=0.0,
                           use_multi_scale=True, revin=1, affine=0,
                           subtract_last=0, freq="s", seq_len=128, c_in=2,
                           individual=0, target_window=32,
                            class_drop=0.0, class_num=10)

    sgn = torch.randn((3,2,128))
    output = model.classification(sgn)
    print(output.shape)